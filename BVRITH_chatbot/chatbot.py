...
class CollegeChatbot:
    ...
    def answer_question(
        ...
        if social_response:
            ...
            # Extract memories in background — doesn't block the greeting response
            threading.Thread(
                target=self.memory_manager.extract_and_store_memories,
                kwargs=dict(
        ...
        with Timer() as timer:
            ...
            try:
                ...
                response = self.llm.invoke(messages, tools=TOOLS if self._needs_tools(question) else [])
                ...
                # timer.__exit__ hasn't run yet (we're still inside the with block),
                # so timer.duration doesn't exist. Compute elapsed time directly.
                response_time = format_time(time.time() - timer.start)
                return {
                    "answer": answer,
            ...
            # Fire in a background thread so it never blocks the response.
            _user_id = self.current_user_id
            ...
            threading.Thread(
                ...
                kwargs=dict(
                    ...
                    assistant_response=_answer,
                    session_id=_session_id,
                ),
                daemon=True,
            ).start()
            stored = 0  # extraction happens in background; reported as 0 in this response
    ...
    def _needs_tools(self, question: str) -> bool:
        """
        Determine if the question needs tool calls (math, dates, percentages).
        Skipping tools for plain FAQ questions saves ~550 tokens per call.
        """
        q = question.lower()
        tool_keywords = [
            "calculate", "how much", "total fee", "fee for", "cost",
            "percent", "%", "percentage", "how many days", "deadline",
            "days left", "days remaining", "when is", "date", "how long",
            "scholarship", "discount", "installment",
        ]
    ...
    def _rewrite_query(self, question: str) -> str:
        ...
        expansions = {
            ...
            "branch": "B.Tech UG programs departments CSE ECE EEE branches offered BVRIT",
            "branches": "B.Tech UG programs departments CSE ECE EEE branches offered BVRIT",
            "btech": "B.Tech UG programs departments CSE ECE EEE branches offered BVRIT",
            "b.tech": "B.Tech UG programs departments CSE ECE EEE branches offered BVRIT",
            "program": "B.Tech UG programs departments CSE ECE EEE branches offered BVRIT",
            "programs": "B.Tech UG programs departments CSE ECE EEE branches offered BVRIT",
            "department": "department B.Tech UG programs CSE ECE EEE branches offered BVRIT",
            "departments": "department B.Tech UG programs CSE ECE EEE branches offered BVRIT",
            "offer": "B.Tech UG programs departments CSE ECE EEE branches offered BVRIT",
            "offers": "B.Tech UG programs departments CSE ECE EEE branches offered BVRIT",
            "mtech": "M.Tech PG programs postgraduate CSE VLSI Data Sciences BVRIT",
            "m.tech": "M.Tech PG programs postgraduate CSE VLSI Data Sciences BVRIT",
            "pg": "M.Tech PG programs postgraduate CSE VLSI Data Sciences BVRIT",
            "ug": "B.Tech UG programs departments CSE ECE EEE branches offered BVRIT",
        }
    ...
    def _format_context(self, documents: List[Document]) -> str:
        ...
        Caps at 3 chunks to keep the prompt compact and reduce LLM latency.
        """
        context_parts = []
        seen_content = set()
        MAX_CHUNKS = 3  # keep prompt small → faster LLM first-token latency
        for i, doc in enumerate(documents, 1):
            ...
            # Deduplicate by content fingerprint (first 100 chars), not section
            content_key = content[:100]
            if content_key in seen_content:
                continue
            seen_content.add(content_key)

            # Truncate long chunks to keep prompt size manageable (faster LLM response)
            if len(content) > 500:
                content = content[:500] + "..."
    ...
    def _build_tool_messages(
        ...
        # Add chat history (last 3 pairs = 6 messages for context; fewer tokens, faster LLM)
    ...
    def _build_tool_result_messages(
        ...
        # Chat history — last 3 pairs only
        for entry in self.chat_history[-3:]:
            messages.append(HumanMessage(content=entry["question"]))
            messages.append(AIMessage(content=entry["answer"]))
